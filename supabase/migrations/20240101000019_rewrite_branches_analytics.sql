-- Replaces the correlated-subquery implementation with one aggregate pass per
-- transaction table. The signature is unchanged for client compatibility.
...
BEGIN
  ...
  END IF;
  IF v_sort_by NOT IN ('name', 'balance', 'return_rate', 'last_activity') THEN
