-- ============================================
-- M23: ŞUBE ANALİTİĞİ — ŞEHİR/İLÇE FİLTRESİ
-- ============================================
-- Şubeler ekranındaki filtre sheet'i için list_branches_analytics
-- fonksiyonuna p_city_id ve p_district_id parametreleri ekleniyor.
-- Mevcut sözleşme (location sıralama, validasyonlar) korunuyor.
...
BEGIN
  ...
  IF p_district_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.districts WHERE id = p_district_id
  ) THEN
...
REVOKE ALL ON FUNCTION public.list_branches_analytics(
  TEXT, TEXT, DATE, DATE, SMALLINT[], UUID, UUID, TEXT, TEXT, INT, INT
) FROM PUBLIC, anon;
